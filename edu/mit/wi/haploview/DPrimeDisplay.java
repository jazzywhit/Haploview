        ...
        if (Options.isGBrowseShown() && Chromosome.getDataChrom() != null && !Chromosome.getDataChrom().equalsIgnoreCase("none")) {
            try {
                ...
                for (int i = 0; i < numChunks; i++) {
                    ...
                    String imageServer = Chromosome.getDataRelease();
                    if (imageServer.equalsIgnoreCase("none")) {
                        break;
                    }
                    ...
                    while (true) {
                        URL imageUrl = new URL("http://www.hapmap.org/cgi-perl/gbrowse/gbrowse_img/" + imageServer + "/?name=" +
                                gChrom + ":" + chunkLefts[i] + ".." + chunkRights[i] + ";width=" + (int)chunkSpans[i] +
                    ...
                    Image trackImage = toolkit.createImage(baos.toByteArray());
                    MediaTracker mt = new MediaTracker(this);
                    mt.addImage(trackImage, 0);
                    setCursor(new Cursor(Cursor.WAIT_CURSOR));
                    mt.waitForID(0);
                    setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
                    imageVector.add(trackImage);

                    if (trackImage.getHeight(this) > totalHeight) {
                        totalHeight = trackImage.getHeight(this);
                    }
                    totalWidth += trackImage.getWidth(this);
                }
